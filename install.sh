...
# This uses 'sed' to directly edit the main config file, which is more reliable.
# It finds the line starting with '#autologin-user=' and replaces it.
sudo sed -i 's/^#?autologin-user=.*/autologin-user=pi/' /etc/lightdm/lightdm.conf
# It does the same for the session.
